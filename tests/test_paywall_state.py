...
def test_gate_enriches_missing_checkout_and_plan() -> None:
    ...
    respx.post(f"{_BASE}/v1/sdk/limits").mock(
        return_value=httpx.Response(
            200, json={"withinLimits": False, "remaining": 0, "meterName": "requests"}
        )
    )
    respx.post(f"{_BASE}/v1/sdk/checkout-sessions").mock(
        return_value=httpx.Response(
            200,
            json={"sessionId": "cs_x", "checkoutUrl": "https://solvapay.com/c/test"},
        )
    )
    respx.get(f"{_BASE}/v1/sdk/customers/cus_1").mock(
        return_value=httpx.Response(
            200,
            json={
                "reference": "cus_1",
    ...
    client = SolvaPay(api_key="sk_test_dummy", base_url=_BASE)
