...
class MerchantOperations:
    ...
    def get(self) -> Merchant:
        ...
        resp = self._sync.send(
            RequestSpec(method="GET", url=REGISTRY["merchant.get"].path_template, headers=Headers(), json=None)
        )
    ...
    def get_platform_config(self) -> PlatformConfig:
        assert self._sync is not None
        resp = self._sync.send(
            RequestSpec(method="GET", url=REGISTRY["platform.get_config"].path_template, headers=Headers(), json=None)
        )
